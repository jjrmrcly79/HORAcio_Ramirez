...
if (isPost) {
  ...
  try {
    ...
    if (act === 'set_meta') { // V1.5-A: fijar meta del día (misma tabla que /orden)
      if (!(S.es_admin || S.puede_meta)) return J({ ok: false, error: 'No tienes permiso para fijar metas.' });
      const lid = String(body.linea_id || '');
      const orden = String(body.orden || '').trim().slice(0, 300); // 300: soporta lista de OTs (tableros multi_orden)
    ...
    if (act === 'set_pareo') { // V2: planeación asigna un final a un subensamble SMT (1:N)
      ...
      // normaliza con la MISMA regla del sistema (norm_np); guarda claves ya normalizadas
      const nk = await pg(`SELECT horacio.norm_np('${esc(ps)}') AS ns, horacio.norm_np('${esc(pf)}') AS nf`);
      const ns = nk[0].ns, nf = nk[0].nf;
      if (!ns || !nf) return J({ ok: false, error: 'Nombre inválido tras normalizar.' });
      if (ns === nf) return J({ ok: false, error: `"${ps}" queda igual que "${pf}" al normalizar (el sistema quita el sufijo _SMT). Si ${pf} ES el subensamble, usa el botón "es sub".` });
      await pg(`INSERT INTO horacio.pareo_smt(parte_smt,parte_final,fuente,vigente,set_by_panel) VALUES('${esc(ns)}','${esc(nf)}','panel',true,'${esc(by)}') ON CONFLICT (parte_smt,parte_final) DO UPDATE SET vigente=true, fuente='panel', set_by_panel='${esc(by)}', ts=now()`);
      await pg(`DELETE FROM horacio.pareo_excluidos WHERE nkey='${esc(nf)}'`);
      return J({ ok: true });
    }
    if (act === 'mark_subensamble') { // declara una parte pendiente como subensamble SMT (queda seleccionable)
      if (!(S.es_admin || S.puede_pareo)) return J({ ok: false, error: 'Solo planeación.' });
      const p = String(body.parte || '').trim().slice(0, 60);
      if (!p) return J({ ok: false, error: 'Falta la parte.' });
      const nk = await pg(`SELECT horacio.norm_np('${esc(p)}') AS n`);
      const n = nk[0].n;
      if (!n) return J({ ok: false, error: 'Nombre inválido.' });
      await pg(`INSERT INTO horacio.subensambles_smt(parte_smt,set_by_panel) VALUES('${esc(n)}','${esc(by)}') ON CONFLICT (parte_smt) DO NOTHING`);
      await pg(`DELETE FROM horacio.pareo_excluidos WHERE nkey='${esc(n)}'`);
      return J({ ok: true });
    }
    ...
    if (act === 'set_cal_config') { // Consolidación Fase A: modelo de horario del turno (migrado de V2; alimenta meta_sugerida)
      if (!(S.es_admin || S.puede_meta)) return J({ ok: false, error: 'Solo producción/admin edita el horario.' });
      const tm = (x) => (/^([01]\d|2[0-3]):[0-5]\d$/.test(String(x || '')) ? String(x) : null);
      const ti = tm(body.turno_inicio), ci = tm(body.comida_inicio), tf = tm(body.turno_fin);
      ...
      await pg(`UPDATE horacio.calendario_config SET turno_inicio='${ti}',liberacion_min=${Math.round(lib)},comida_inicio='${ci}',comida_min=${Math.round(com)},turno_fin='${tf}',actualizado_ts=now() WHERE id=1`);
      return J({ ok: true });
    }
...
if (q.data === '1') {
  ...
  if (!S) return J({ ok: false, code: 'auth' });
  const personas = await pg("SELECT id, nombre, rol, es_admin, (pin_hash IS NOT NULL) AS has_pin FROM horacio.personas WHERE activa ORDER BY (rol='lider') DESC, rol, nombre");
  const tableros = await pg(`SELECT l.id, l.codigo, l.nombre, l.grupo, l.orden, l.unidad, l.captura, l.supervisor_rol, l.lider_persona_id, l.multi_orden, p.nombre AS lider, (SELECT o.orden FROM horacio.ordenes_tablero o WHERE o.linea_id=l.id AND o.fecha='${fecha}' AND o.vigente ORDER BY o.ts DESC LIMIT 1) AS ot_hoy, (SELECT o.meta_hr FROM horacio.ordenes_tablero o WHERE o.linea_id=l.id AND o.fecha='${fecha}' AND o.vigente ORDER BY o.ts DESC LIMIT 1) AS meta_hoy, (SELECT o.orden FROM horacio.ordenes_tablero o WHERE o.linea_id=l.id AND o.fecha='${fechaManana}' AND o.vigente ORDER BY o.ts DESC LIMIT 1) AS ot_man, (SELECT o.meta_hr FROM horacio.ordenes_tablero o WHERE o.linea_id=l.id AND o.fecha='${fechaManana}' AND o.vigente ORDER BY o.ts DESC LIMIT 1) AS meta_man, (EXISTS(SELECT 1 FROM horacio.linea_proceso lp WHERE lp.linea_codigo=l.codigo)) AS mapeada FROM horacio.lineas l LEFT JOIN horacio.personas p ON p.id=l.lider_persona_id WHERE l.activa ORDER BY l.grupo, l.orden`);
  ...
  if (S.es_admin || S.puede_pareo) {
    ...
    subensambles = await pg("SELECT parte_smt FROM (SELECT DISTINCT parte_smt FROM horacio.pareo_smt WHERE vigente UNION SELECT parte_smt FROM horacio.subensambles_smt) u ORDER BY parte_smt");
  }
  // calendario/horario del turno (config; migrado de V2) — producción/admin
  let calCfg = null;
  if (S.es_admin || S.puede_meta) {
    const cc = await pg("SELECT to_char(turno_inicio,'HH24:MI') AS turno_inicio, liberacion_min, to_char(comida_inicio,'HH24:MI') AS comida_inicio, comida_min, to_char(turno_fin,'HH24:MI') AS turno_fin FROM horacio.calendario_config WHERE id=1");
    calCfg = (cc && cc[0]) ? cc[0] : null;
  }
  return J({
...
'var S=ssGet("panel_s"), ME=null, ST=null, TAB="captura", PRE=null, PREC=null, WHO=null, GP=null, SUG={}, METAF="hoy", MORD={};',
...
'  if(TAB=="captura")return renderMatriz(v);if(TAB=="registrar")return renderReg(v);if(TAB=="metas")return renderMetas(v);if(TAB=="estandar")return renderEstandar(v);if(TAB=="pareo")return renderPareo(v);if(TAB=="calendario")return renderCalendario(v);if(TAB=="tableros")return renderTableros(v);if(TAB=="asignar")return renderAsignar(v);if(TAB=="paros")return renderParos(v);if(TAB=="personas")return renderPersonas(v);if(TAB=="perfiles")return renderPerfiles(v);}',
...
// ----- Metas de tablero con VARIAS órdenes (multi_orden, ej. Embarques) -----
'function metaMultiRow(t,ot,mt,map,man){var k=t.id+(man?"_m":"_h");if(MORD[k]===undefined)MORD[k]=(ot?String(ot).split(",").map(function(x){return x.trim();}).filter(Boolean):[]);return "<div class=\\"tline\\" style=\\"display:block\\"><div class=\\"field\\"><label>"+h(t.nombre)+map+" <span class=\\"muted\\">(varias \\u00f3rdenes \\u00b7 una meta)</span></label></div><div id=\\"moc_"+k+"\\" style=\\"display:flex;flex-wrap:wrap;gap:6px;margin:6px 0\\">"+moChips(k)+"</div><div class=\\"row\\" style=\\"align-items:flex-end;gap:8px\\"><div class=\\"field\\" style=\\"flex:1;min-width:150px\\"><label>Agregar orden</label><select id=\\"moa_"+k+"\\">"+otOpts(t,"")+"</select></div><button class=\\"btn sm\\" onclick=\\"moAdd(\\x27"+k+"\\x27)\\">+ agregar</button><div class=\\"field\\"><label>Meta/hr (tablero)</label><input id=\\"mm_"+k+"\\" inputmode=\\"numeric\\" style=\\"width:100px\\" value=\\""+mt+"\\"></div><button class=\\"btn primary sm\\" onclick=\\"doMetaMulti(\\x27"+t.id+"\\x27,"+man+")\\">guardar</button> <span class=\\"muted\\" id=\\"ms_"+k+"\\"></span></div></div>";}',
'function moChips(k){var a=MORD[k]||[];return a.length?a.map(function(o,i){return "<span class=\\"pill\\" style=\\"background:#e8effc;border:1px solid #c7d2fe\\">"+h(o)+" <a href=\\"#\\" onclick=\\"moDel(\\x27"+k+"\\x27,"+i+");return false;\\" style=\\"color:#b91c1c;text-decoration:none;font-weight:700\\">\\u00d7</a></span>";}).join(""):"<span class=\\"muted\\">sin \\u00f3rdenes a\\u00fan</span>";}',
'function moAdd(k){var s=document.getElementById("moa_"+k);var v=(s.value||"").trim();if(!v)return;MORD[k]=MORD[k]||[];if(MORD[k].indexOf(v)<0)MORD[k].push(v);s.value="";document.getElementById("moc_"+k).innerHTML=moChips(k);}',
'function moDel(k,i){if(!MORD[k])return;MORD[k].splice(i,1);document.getElementById("moc_"+k).innerHTML=moChips(k);}',
'async function doMetaMulti(id,man){var k=id+(man?"_m":"_h");var orden=(MORD[k]||[]).join(", ");var m=document.getElementById("mm_"+k).value;if(!m){tj("Escribe la meta del tablero");return;}var d=await post({action:"set_meta",linea_id:id,orden:orden,meta:m,fecha:(man?(ST.fecha_manana||""):(ST.fecha||""))});if(d.ok){tj("Guardado \\u2713");delete MORD[k];await load();go("metas");}else if(d.code=="auth"){logout();}else tj(d.error||"no se pudo");}',
...
'  var dl="<datalist id=\\"estdl\\">"+EP.map(function(p){return "<option value=\\""+h(p.np+(p.desc?" | "+String(p.desc).slice(0,50):""))+"\\"></option>";}).join("")+"</datalist>";',
'  var sel="<input id=\\"estpick\\" list=\\"estdl\\" autocomplete=\\"off\\" placeholder=\\"escribe n\\u00famero de parte o descripci\\u00f3n\\u2026\\" style=\\"min-width:340px;padding:8px 10px;border:1px solid var(--bd);border-radius:9px\\">"+dl;',
'  var chips=prio.length?("<div style=\\"margin:8px 0 4px\\"><span class=\\"muted\\">\\u26a0 Prioridad (sin est\\u00e1ndar, en OT):</span> "+prio.slice(0,24).map(function(p){return "<button class=\\"btn sm\\" style=\\"margin:2px\\" onclick=\\"estPick(\\x27"+h(p.np)+"\\x27)\\">"+h(p.np)+"</button>";}).join("")+(prio.length>24?" <span class=\\"muted\\">+"+(prio.length-24)+" m\\u00e1s</span>":"")+"</div>"):"";',
'  v.innerHTML="<div class=\\"card\\"><h2>Capturar / editar est\\u00e1ndar por hora</h2><div class=\\"muted\\" style=\\"margin-bottom:10px\\">Llena el Std/Hr (pz/hr te\\u00f3ricas) de cada estaci\\u00f3n. Se guarda al salir del campo. Vac\\u00edo = borra. Las partes \\u26a0 <b>de OT en proceso</b> son prioridad: al llenarlas, su meta sale sola.</div>"+chips+"<div class=\\"row\\">"+sel+" <span id=\\"estinfo\\" class=\\"muted\\"></span></div><div id=\\"estgrid\\" style=\\"margin-top:12px\\"></div></div>";',
'  var pick=document.getElementById("estpick");pick.onchange=function(){var vv=pick.value.trim();var np=vv.split(" | ")[0].trim();estGrid(np);};if(prio.length){pick.value=prio[0].np;estGrid(prio[0].np);}}',
'function estPick(np){var el=document.getElementById("estpick");if(el)el.value=np;estGrid(np);}',
...
'async function doEsSub(i){var p=(ST.pareoPend||[])[i];if(!p)return;if(!confirm("\\u00bfMarcar "+p.np+" como SUBENSAMBLE SMT? Queda disponible para parearle sus finales."))return;var d=await post({action:"mark_subensamble",parte:p.np});if(d.ok){tj("Es subensamble \\u2713");await load();go("pareo");}else if(d.code=="auth"){logout();}else tj(d.error||"no se pudo");}',
...
// ----- Consolidación Fase A: Calendario / horario del turno (migrado de V2, solo captura) -----
'function calMin(s){var p=String(s||"0:0").split(":");return (parseInt(p[0],10)||0)*60+(parseInt(p[1],10)||0);}',
'function calProd(){var t=document.getElementById("cf_ti");if(!t)return;var ti=calMin(t.value),tf=calMin(document.getElementById("cf_tf").value),lib=parseInt(document.getElementById("cf_lib").value,10)||0,com=parseInt(document.getElementById("cf_com").value,10)||0;var prod=(tf-ti)-lib-com;var el=document.getElementById("cf_prod");if(el)el.innerHTML=prod>0?"Hoy: <b>"+(prod/60).toFixed(1)+" h productivas/d\\u00eda</b>":"<span style=\\x27color:#b91c1c\\x27>Revisa las horas.</span>";}',
'function renderCalendario(v){if(!(ME&&(ME.es_admin||ME.puede_meta))){v.innerHTML="<div class=\\"card\\"><div class=\\"muted\\">Solo producci\\u00f3n/admin edita el horario.</div></div>";return;}',
'  var c=ST.calCfg||{turno_inicio:"06:30",liberacion_min:60,comida_inicio:"12:00",comida_min:30,turno_fin:"15:30"};',
'  v.innerHTML="<div class=\\"card\\"><h2>Modelo de horario del turno</h2><div class=\\"muted\\" style=\\"margin-bottom:10px\\">Horas productivas = (fin \\u2212 inicio) \\u2212 liberaci\\u00f3n \\u2212 comida. Alimenta la <b>sugerencia de meta</b> (prorrateo). Cambia y guarda.</div><div class=\\"row\\" style=\\"gap:10px;flex-wrap:wrap;align-items:flex-end\\"><div class=\\"field\\"><label>Inicio turno</label><input id=\\"cf_ti\\" type=\\"time\\" oninput=\\"calProd()\\" value=\\""+h(c.turno_inicio)+"\\"></div><div class=\\"field\\"><label>Liberaci\\u00f3n (min)</label><input id=\\"cf_lib\\" type=\\"number\\" min=\\"0\\" max=\\"240\\" oninput=\\"calProd()\\" value=\\""+h(c.liberacion_min)+"\\" style=\\"width:90px\\"></div><div class=\\"field\\"><label>Comida inicia</label><input id=\\"cf_ci\\" type=\\"time\\" value=\\""+h(c.comida_inicio)+"\\"></div><div class=\\"field\\"><label>Comida (min)</label><input id=\\"cf_com\\" type=\\"number\\" min=\\"0\\" max=\\"180\\" oninput=\\"calProd()\\" value=\\""+h(c.comida_min)+"\\" style=\\"width:90px\\"></div><div class=\\"field\\"><label>Fin turno</label><input id=\\"cf_tf\\" type=\\"time\\" oninput=\\"calProd()\\" value=\\""+h(c.turno_fin)+"\\"></div><button class=\\"btn primary\\" onclick=\\"doCalCfg()\\">Guardar</button> <span class=\\"muted\\" id=\\"cf_msg\\"></span></div><div class=\\"muted\\" id=\\"cf_prod\\" style=\\"margin-top:10px\\"></div></div>";calProd();}',
'async function doCalCfg(){var b={action:"set_cal_config",turno_inicio:document.getElementById("cf_ti").value,liberacion_min:document.getElementById("cf_lib").value,comida_inicio:document.getElementById("cf_ci").value,comida_min:document.getElementById("cf_com").value,turno_fin:document.getElementById("cf_tf").value};var d=await post(b);if(d.ok){tj("Horario guardado \\u2713");await load();go("calendario");}else if(d.code=="auth"){logout();}else tj(d.error||"no se pudo");}',
...
'try{setWho();if(S){load();}else{showLogin();}setInterval(function(){if(S&&(TAB=="captura"||TAB=="paros"))load();},30000);setInterval(function(){if(TAB=="paros")tickParos();},1000);}catch(e){document.getElementById("sub").textContent="error al iniciar: "+e.message;}',
'</script></body></html>'
].join('');
